...
//  oled.setTextSize(2);                  // at double size character
  ...
  oled.println(F("SYED’s"));          // this for SH1106
...
void dmm50V() {                            //  5V  digital voltmeter 5V range
  ...
  while (1) {                              // forever,
    ...
    if (digitalRead(7) == HIGH) {          // DC   if awitch is DC mode
      ...
      vPP = (dataMax - dataMin) * lsb50Vac; // P-P get Peak to peak voltage
...
#endif
{
  ...
  if ((x & 0x10) == 0) {           // if DOWN button(Pin12) pusshed, and
    if (scopeP == 0) {             // scoped vertical range
      vRange--;                    // V-range DOWN
      if (vRange < 2) {            // if bottom (0,1 Auto5V Auto50V
        vRange = 2;                // stay as is
      }
    }
    if (scopeP == 1) {             // if scoped hrizontal range
...
// OCR1A Fast PWM Mode, TOP=OCR1A (WGM11:10 = 0b11)
...
// OCR1A Fast PWM Mode (WGM13:12 = 0b11)
// CS12:10 001:ck/1, 010:ck/8, 011:ck/64, 100:ck/256, 101:ck/1024
  TCCR1B = _BV(WGM13) | _BV(WGM12) | _BV(CS10); // 001:ck/1 
//  TCCR1B = 0b00011001;  // ck/1
